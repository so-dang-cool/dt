use crate::dt_machine::{self, Definition, DtType, DtValue, Stack};
...
pub fn builtins() -> Vec<Definition<'static>> {
    vec![
        Definition::on_state("len", &[QuoteOrString], &[I64], |quote| {
            let (a, quote) = quote.pop();
            let len: i64 = match a {
                DtValue::Quote(quote) => quote.len(),
                DtValue::String(s) => s.len(),
                _ => {
        ...
        Definition::on_state("quote", &[A], &[Quote], |quote| {
            let (a, quote) = quote.pop();
            let wrapper = quote.child();
            let wrapper = wrapper.push(a);
            quote.push_quote(wrapper)
        }),
        Definition::on_state("unquote", &[Quote], &[Unknown], |quote| {
        ...
        Definition::on_state("deq", &[Quote], &[A, Quote], |quote| {
            let (sequence, quote) = quote.pop_quote("pop");
            let (a, sequence) = sequence.dequeue();
            quote.push(a).push_quote(sequence)
        }),
        Definition::on_state("rev", &[QuoteOrString], &[Quote], |quote| {
            let (a, quote) = quote.pop();
            match a {
                DtValue::String(s) => quote.push_string(s.chars().rev().collect()),
                DtValue::Quote(q) => quote.push_quote(q.reverse()),
                _ => {
                    dt_machine::log_warn(format!(
                        "Can only perform len on quote or string but got {}",
                        a
        ...
        Definition::on_state("concat", &[Quote, Quote], &[Quote], |quote| {
        ...
        Definition::on_state("filter", &[Quote, Quote], &[Quote], |state| {
        ...
        Definition::on_state("map", &[Quote, Quote], &[Quote], |state| {
        ...
        Definition::on_state("each!", &[Quote, Quote], &[], |state| {
        ...
        Definition::on_jailed_state("each", &[Quote, Quote], &[], |state| {
            let (command, quote) = state.stack.clone().pop_quote("each");
            let (sequence, quote) = quote.pop_quote("each");
        ...
        Definition::on_state("zip", &[Quote, Quote], &[Quote], |state| {
        ...
        Definition::on_state("zip-with", &[Quote, Quote, Quote], &[Quote], |state| {
